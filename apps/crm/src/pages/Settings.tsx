...
import { UsersSettingsPanel, EmailTemplatesSettingsPanel } from '@kodan-apps/ui-core'
...
import { GitBranch, Settings2, Users, BellRing, ListTodo, Workflow, Mail } from 'lucide-react'

type SettingsPanel = 'users' | 'pipelines' | 'custom-fields' | 'notifications' | 'task-types' | 'workflows' | 'email-templates'

function getSectionFromHash(): SettingsPanel | null {
  const hash = window.location.hash.replace('#', '')
  if (hash === 'users' || hash === 'pipelines' || hash === 'custom-fields' || hash === 'notifications' || hash === 'task-types' || hash === 'workflows' || hash === 'email-templates') return hash
  return null
}
