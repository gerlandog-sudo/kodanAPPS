...
Genera un veredicto corto (max 40 palabras) en espanol sobre el impacto en tiempo y rentabilidad.`;
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-8 bg-white rounded-2xl border border-gray-100 h-48">
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {alerts.map((alert, idx) => (
          <div key={alert.projectId} className={`p-5 rounded-2xl border-l-4 shadow-sm relative overflow-hidden bg-white ${alert.priority === 'High' ? 'border-red-500' : alert.priority === 'Medium' ? 'border-amber-500' : 'border-primary'}`}>
            <div className="flex justify-between items-start mb-3">
              <div>
                ...
                <span className={`text-[10px] px-2 py-0.5 rounded-lg font-bold uppercase tracking-wider border inline-block mt-1 ${alert.priority === 'High' ? 'bg-red-50 text-red-600 border-red-200' : alert.priority === 'Medium' ? 'bg-amber-50 text-amber-600 border-amber-200' : 'bg-blue-50 text-blue-600 border-blue-200'}`}>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              ...
              <div className="p-2.5 rounded-xl bg-gray-50 col-span-2">
                ...
                <div className="h-1.5 rounded-lg bg-gray-200 overflow-hidden">
            ...
            <div className={`p-3 rounded-xl border ${loadingInsights[alert.projectId] ? 'animate-pulse bg-gray-50 border-gray-200' : 'bg-primary/5 border-primary/20'}`}>
              {loadingInsights[alert.projectId] ? (
                ...
                <div className="space-y-3">
                  ...
                  <button onClick={() => setSimAlert({ projectName: alert.projectName, index: idx, metrics: alert.metrics })} className="flex items-center gap-1.5 text-xs text-white bg-primary hover:bg-primary/90 px-5 py-2.5 rounded-xl shadow-sm">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden border border-gray-100">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <div className="flex items-center gap-3"><div className="p-2 bg-primary/10 rounded-xl text-primary"><Calculator size={20} /></div><h3 className="font-bold text-gray-900">Simulación de Cambio</h3></div>
              <button onClick={() => setSimAlert(null)} className="p-2 hover:bg-gray-100 rounded-lg text-gray-400"><X size={20} /></button>
            </div>
            ...
            <div className="p-6 bg-gray-50/50 border-t border-gray-100 flex gap-3">
              ...
              <button onClick={handleSimulate} disabled={simulating} className="flex-1 px-6 py-4 bg-primary hover:bg-primary/90 disabled:opacity-50 text-white rounded-2xl font-bold flex items-center justify-center gap-2 shadow-lg uppercase text-xs tracking-widest">
