...
import { PaginationBar } from './TablePagination'
import { SkeletonBar } from './TableSkeleton'
import './Table.css'
...
export function Table<T>({
  ...
  const handleSort = (key: keyof T) => {
    ...
    setSortKey(key)
    setSortDir(newDir)
    if (onSort) onSort(key as string, newDir)
  }

  const sortedData = useMemo(() => {
    if (!sortKey || onSort) return displayData
    return [...displayData].sort((a: T, b: T) => {
              ...
              {columns.map(col => (
                <th
                  ...
                  {...(col.sortable ? {
                    ...
                    'tabIndex': 0 as const,
                    'onKeyDown': (e: React.KeyboardEvent) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handleSort(col.key as keyof T) } },
                  } : {})}
                >
